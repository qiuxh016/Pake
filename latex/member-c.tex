...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item \textbf{通过全局快捷键打开历史面板。}
    功能启用后，\path{set_global_shortcut} 在 Windows/Linux 注册 Ctrl+Shift+V，在 macOS 注册
    Cmd+Shift+V。若它与应用唤醒快捷键相同，则剪贴板历史优先；回调还通过
    \path{Instant} 设置 300 ms 防抖。触发后 \path{toggle_panel} 创建或复用
    \path{clipboard-panel} Webview：初始大小为 $320\times480$，最小为 $320\times360$，
    窗口无边框、置顶、可缩放且不显示在任务栏，并根据当前显示器尺寸定位到右下角。
    面板已存在时只刷新内容并获取焦点，重复按快捷键不会误将其关闭。
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item \textbf{全局快捷键可能重复触发或与应用唤醒快捷键冲突}
    ...
    面板采用独立、置顶、无边框且可缩放的 Webview，重复按快捷键只聚焦窗口，不误关闭面板。
\end{enumerate}
...
实际测试中先执行 \texttt{pnpm run cli:build} 生成最新 CLI，随后使用
\texttt{--clipboard --clipboard-max 500} 构建启用剪贴板管理的 Windows 应用。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{images/powershell-a-build.png}
  \\[0.25cm]
  \includegraphics[width=0.88\textwidth]{images/powershell-a-release.png}
  \caption{PowerShell A：CLI 构建、打包命令与 release 产物生成}
\end{figure}
